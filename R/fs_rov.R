...
fs_diff_sampler <- function(fs_set) {
  ...
  pair <- sort(sample((unique(fs_set$sim)), size = 2, replace = F))
...
rep_diff_sampler <- function(fs_set, ndraws) {
  ...
  diff_df <- diff_df %>%
    dplyr::distinct_at(c("s0", "n0", "sim1", "sim2", "r2", "r2_log", "cd", "prop_off", "div")) %>%
    dplyr::mutate(
      source = fs_set$source[1],
      dat = fs_set$dat[1],
      site = fs_set$site[1],
      singletons = fs_set$singletons[1]
    )
...
#' Summarize diffs
#'
#' @param diffs diffs df
#'
#' @return summarized
#' @export
#' @importFrom dplyr mutate group_by summarize ungroup
summarize_diffs <- function(diffs) {
  diffs_summary <- diffs %>%
    dplyr::mutate(sim_name = paste0(sim1, "_", sim2)) %>%
    dplyr::group_by(s0, n0, source, dat,site, singletons) %>%
    dplyr::summarize(
      n_comparisons = length(unique(sim_name)),
      mean_r2 = mean(r2),
