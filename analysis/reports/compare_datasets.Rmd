...
overall <- ggplot(data = filter(all_statevars, !singletons), aes(x = s0, y = n0)) +
  geom_point(alpha = .3) +
  theme_bw() +
...
max_min_s0 = max(all_extremes$min_s0)
max_min_n0 = max(all_extremes$min_n0)

min_max_s0 = min(all_extremes$max_s0)
min_max_n0 = min(all_extremes$max_n0)

all_statevars_within <- all_statevars %>%
  filter(s0 > max_min_s0, s0 < min_max_s0, n0 > max_min_n0, n0 < min_max_n0, !singletons) %>%
  select(site, dat, s0, n0) %>%
  group_by(s0, n0) %>%
  mutate(ndats = length(unique(dat)),
         nsites = dplyr::n()) %>%
  ungroup()

new_occ <- ggplot(data = all_statevars_within, aes(x = s0,y = n0, color = as.factor(ndats))) +
  geom_point(alpha = .3) +
  scale_color_viridis_d(option = "plasma", end = .9, direction = -1) +
  theme_bw()
new_occ
```
