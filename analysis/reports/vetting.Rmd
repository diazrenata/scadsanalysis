...
sum(filter(all_di, nsamples >= 2000, singletons == F)$skew_percentile >= 95) / nrow(filter(all_di, nsamples >= 2000, singletons == F))
```
...
ggplot(data = filter(all_di, singletons == FALSE), aes(x = log(s0), y = log(n0), color = skew_percentile)) +
  geom_point(alpha = .1) +
  theme_bw() +
  scale_color_viridis_c(option = "plasma", end = .8)

ggplot(data = filter(all_di, singletons == FALSE, nsamples >= 2000), aes(x = log(s0), y = log(n0), color = skew_percentile)) +
  geom_point(alpha = .25) +
  theme_bw() +
  scale_color_viridis_c(option = "plasma", end = .8)


ggplot(data = filter(all_di, singletons == FALSE), aes(x = log(s0), y = log(n0), color = simpson_percentile)) +
  geom_point(alpha = .1) +
  theme_bw() +
  scale_color_viridis_c(option = "plasma", end = .8, direction = -1)

ggplot(data = filter(all_di, singletons == FALSE, nsamples >= 2000), aes(x = log(s0), y = log(n0), color = simpson_percentile)) +
  geom_point(alpha = .25) +
  theme_bw() +
  scale_color_viridis_c(option = "plasma", end = .8, direction = -1)
```
