...
all_di <- lapply(dis, FUN = function(di_df) return(read.csv(here::here("analysis", "results", di_df), stringsAsFactors = F)))
...
all_di <- all_di %>%
  ...
  mutate(found_all = prop_found==1) %>% 
    mutate(binned_nparts = as.factor(ceiling(log_nparts/5)))


diffs <- list.files(here::here("analysis", "results"), pattern = "diff_")
all_diffs <- lapply(diffs, FUN = function(di_df) return(read.csv(here::here("analysis", "results", di_df), stringsAsFactors = F)))
