...
```{r nparts variability}

ggplot(filter(all_di, !singletons, s0 >= 3, !is.infinite(skew_sd), !is.infinite(skew_mean), nsamples >2), aes(x = log_nparts, y = (skew_sd/skew_mean), color = skew_percentile)) +
  geom_point(alpha = .1) +
  theme_bw() +
...
ggplot(filter(all_di, !singletons, s0 >= 3, !is.infinite(simpson_sd), !is.infinite(simpson_sd), nsamples >2), aes(x = log_nparts, y = (simpson_sd/simpson_mean), color = simpson_percentile)) +
  geom_point(alpha = .1) +
  theme_bw() +
  geom_vline(xintercept = 10) +
  scale_color_viridis_c() +
...
ggplot(filter(all_di, !is.infinite(skew_mean), !is.infinite(skew_sd)), aes(x = skew_mean, y = skew_sd)) +
  geom_point() +
  theme_bw()

all_di <- all_di %>%
  mutate(skew_cv = skew_sd / abs(skew_mean),
         simpson_cv = simpson_sd / abs(simpson_mean))
...
ggplot(filter(all_di, !is.infinite(simpson_mean), !is.infinite(simpson_sd)), aes(x = simpson_mean, y = simpson_sd)) +
  geom_point() +
  theme_bw() +
  scale_color_viridis_c()
...
ggplot(filter(all_di, !singletons, !is.infinite(skew_mean), s0 >= 3, !is.infinite(skew_sd), nparts > 2), aes(log(s0), log(n0), color = log(skew_cv))) +
  geom_point(alpha = .15) +
...
ggplot(filter(all_di, !singletons, !is.infinite(simpson_mean), s0 >= 2, !is.infinite(simpson_mean), nparts > 2), aes(log(s0), log(n0), color = log(simpson_cv))) +
  geom_point(alpha = .15) +
  scale_color_viridis_c(option = "plasma", end = .9) +
  theme_bw() 
# +
...
```{r cv v nparts}
ggplot(filter(all_di, !singletons, !is.infinite(skew_mean), s0 >= 3, !is.infinite(skew_sd), nparts > 2), aes(x = log_nparts, y = skew_cv)) +
  geom_point(alpha = .05) +
  theme_bw() +
  geom_vline(xintercept = 10) +
  xlim(0, 75)
...
ggplot(filter(all_di, !singletons, !is.infinite(simpson_mean), s0 >= 2, !is.infinite(simpson_mean), nparts > 2), aes(x = log_nparts, y = simpson_cv)) +
  geom_point(alpha = .05) +
  theme_bw() +
  geom_vline(xintercept = 10) +
  xlim(0, 75)
...
all_di <- all_di %>%
  group_by_all() %>%
  mutate(log_nparts_thresh = min(log_nparts, 5)) %>%
  ungroup()

ggplot(filter(all_di, !singletons, !is.infinite(skew_mean), s0 >= 3, !is.infinite(skew_sd), nparts > 2), aes(y = skew_cv, x = skew_percentile, color = log_nparts_thresh)) +
  geom_point(alpha = .1) +
  theme_bw() +
  facet_wrap(vars(more_than_10)) +
  scale_color_viridis_c()
...
ggplot(filter(all_di, !singletons, !is.infinite(simpson_mean), s0 >= 2, !is.infinite(simpson_sd), nparts > 2), aes(y = simpson_cv, x = simpson_percentile, color = log_nparts_thresh)) +
  geom_point(alpha = .1) +
  theme_bw() +
  facet_wrap(vars(more_than_10)) +
  scale_color_viridis_c() 
