...
ggplot(filter(fia_sized, s0 > 2, nparts > 50), aes(skew_percentile_excl)) +
  geom_histogram() +
  facet_wrap(vars(fia_yn), scales = "free_y")


ggplot(filter(fia_sized, s0 > 2), aes(skew_percentile_excl)) +
  geom_histogram() +
  facet_wrap(vars(fia_yn), scales = "free_y")
...
ggplot(filter(fia_sized, s0 > 2, nparts > 50, nparts < exp(7)), aes(x = log(s0), y = log(n0), color = fia_yn)) +
  geom_point(alpha = .2) +
  facet_wrap(vars(fia_yn))

ggplot(filter(fia_sized, s0 > 2, nparts >= exp(7), nparts < exp(10)), aes(skew_percentile)) +
  geom_histogram(bins = 50) + theme_bw() +
  facet_wrap(vars(fia_yn), scales = "free_y") +
  ggtitle("1,000 - 20,000 parts in FS")

ggplot(filter(fia_sized, s0 > 2, nparts >= exp(7), nparts < exp(10)), aes(x = log(s0), y = log(n0), color = fia_yn)) +
  geom_point(alpha = .2) +
  facet_wrap(vars(fia_yn))
