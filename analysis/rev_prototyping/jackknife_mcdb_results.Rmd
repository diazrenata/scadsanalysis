...
jk_di <- read.csv(here::here("analysis", "rev_prototyping", "mcdb_di_jk.csv"))
...
wide_di <- left_join(jk_di, all_di) %>%
  ...
  mutate(mean_skew_percentile_excl = mean(skew_percentile_excl),
         ...
         mean_shannon_percentile = mean(shannon_percentile),
  ...
  mutate(actual_sig_skew = skew_percentile_excl_actual > 95,
         ...
         actual_sig_shannon = shannon_percentile_actual < 5,
         mean_sig_shannon=  mean_shannon_percentile < 5) %>%
  mutate(skew_change = paste0("actual", actual_sig_skew, "_jk", mean_sig_skew),
         ...
         shannon_change = paste0("actual", actual_sig_shannon, "_jk", mean_sig_shannon))

```

```{r}

wide_di_small <- wide_di %>%
  select(site_source, mean_skew_percentile_excl, mean_simpson_percentile, s0, n0, mean_nparts, skew_change, even_change, actual_sig_skew, actual_sig_even, mean_sig_skew, mean_sig_even, mean_shannon_percentile, actual_sig_shannon, mean_sig_shannon, shannon_change) %>%
  filter(s0 > 2,
         s0 != n0,
         n0 != s0 + 1) %>%
  distinct()

ggplot(wide_di_small, aes(mean_skew_percentile_excl)) +
  geom_histogram()


ggplot(wide_di_small, aes(mean_simpson_percentile)) +
  geom_histogram()

ggplot(wide_di_small, aes(mean_shannon_percentile)) +
  geom_histogram()

mean(wide_di_small$actual_sig_skew)
mean(wide_di_small$mean_sig_skew)

mean(wide_di_small$actual_sig_shannon)
mean(wide_di_small$mean_sig_shannon)

mean(wide_di_small$actual_sig_even)
mean(wide_di_small$mean_sig_even)

wide_di_small %>%
  group_by(even_change) %>%
  tally()
