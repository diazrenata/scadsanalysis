...
all <- drake_plan(
  ...
  fs = target(sample_fs_wrapper(dataset = dat_s_dat_portal_plants, site_name = s, singletonsyn = singletons, n_samples = ndraws, p_table = tall_p, seed = !!sample.int(10^6, size = 1)),
              transform = cross(s = !!sites_list$site,
                                singletons = !!c(FALSE, TRUE))),
  di = target(add_dis(fs),
              transform = map(fs)),
  all_di = target(dplyr::bind_rows(di), transform = combine(di)),
...
db <- DBI::dbConnect(RSQLite::SQLite(), here::here("analysis", "drake", "drake-cache-kde.sqlite"))
...
# } else {
  ...
  system.time(make(all, cache = cache, cache_log_file = here::here("analysis", "drake", "cache_log_kde.txt"), parallelism = "clustermq", jobs = 2))
#}
