...
dataset <- read.csv(here::here("analysis", "rev_prototyping", "jacknifed_datasets", "bbs_jk.csv"))
...
all <- drake_plan(
  dat = target(read.csv(here::here("analysis", "rev_prototyping", "jacknifed_datasets", "bbs_jk.csv")),
               hpc = F
  ),
...
db <- DBI::dbConnect(RSQLite::SQLite(), here::here("analysis", "drake", "drake-cache-bbs-jk.sqlite"))
...
# } else {
  ...
  system.time(make(all, cache = cache, cache_log_file = here::here("analysis", "drake", "cache_log_bbs_jk.txt"), verbose = 1, memory_strategy = "autoclean"))
#}
