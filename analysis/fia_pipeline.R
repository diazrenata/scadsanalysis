...
write.csv(all_summaries, here::here("analysis", "results", "diff_summaries_fia_short.csv"), row.names = F)
rm(diffs_summaries)
rm(all_summaries)
all_di <- readd(all_di_obs, cache = cache)
write.csv(all_di, here::here("analysis", "results", "all_di_fia_short.csv"), row.names = F)
rm(all_di)
